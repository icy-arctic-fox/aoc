...
class Grid
  ...
  def resize(width, height) : Nil
    ...
    @grid = BitArray.new((width * height).to_i32) do |index|
...
loop do
  ...
  sim.update
  puts sim.shape_count.humanize if DEBUG > 0 && sim.shape_count.divisible_by?(1000)
  break if sim.shape_count >= 1000000000000
