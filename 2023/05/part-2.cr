...
record RangeMap, source_start : Int64, destination_start : Int64, size : Int64 do
  ...
  def destination_includes?(value)
    destination_start <= value <= (destination_start + size)
  end
  ...
  def reverse_map(value)
    return value unless destination_includes?(value)
...
almanac = Almanac.new
seeds = [] of Range(Int64, Int64)

STDIN.each_line do |line|
  next if line.empty?

  if line.starts_with?("seeds:")
    pairs = line.split(':').last.scan(/\d+/).map &.[0].to_i64
    pairs.each_slice(2) do |(start, size)|
...
chain = Iterator.chain(seeds.map &.each)
min = chain.min_of { |seed| almanac.map("seed", "location", seed) }
puts min
