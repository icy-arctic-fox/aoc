...
base = Grid.from_io(STDIN)

top = (0...(base.width)).map { |x| {x, 0, Direction::Down} }
bottom = (0...(base.width)).map { |x| {x, base.height - 1, Direction::Up} }
left = (0...(base.height)).map { |y| {0, y, Direction::Right} }
right = (0...(base.height)).map { |y| {base.width - 1, y, Direction::Left} }

max = (top + bottom + left + right).max_of do |args|
  grid = base.dup
  grid.simulate(*args)
  grid.energized
end
