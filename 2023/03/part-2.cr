...
class Schematic
  ...
  def neighbor_numbers(x : Int32, y : Int32) : Enumerable(Int32)
  ...
  def each_gear(& : Int32, Int32, Int32, Int32 -> _)
    each_symbol do |x, y, char|
      next unless char == '*'

      numbers = neighbor_numbers(x, y)
...
sum = 0
schematic.each_gear do |x, y, r1, r2|
  STDERR.puts "(#{x}, #{y}) #{r1}x#{r2}"
  sum += r1 * r2
end
puts sum
