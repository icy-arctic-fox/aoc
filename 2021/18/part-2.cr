...
abstract class Node
  ...
  def +(other : Node)
    node = TupleNode.new(clone, other.clone)
...
class TupleNode < Node
  ...
  def clone
    TupleNode.new(@left.clone, @right.clone)
  end
...
nodes = STDIN.each_line(chomp: true).map do |line|
  reader = NodeReader.new(line)
  reader.read
end.to_a
max = nodes.each_permutation(2).max_of do |pair|
  a, b = pair
  (a + b).magnitude
end
puts max
