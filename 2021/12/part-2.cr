...
def reject?(path, c)
  return true if c.name.in?(START, FINISH) && path.includes?(c)

  small = (path + [c]).select(&.small?)
  small.size - small.uniq.size > 1
end
...
def accept?(path, c)
  ...
  return false if complete.count { |s| s.name == START } > 1
  return false if complete.count { |s| s.name == FINISH } > 1
